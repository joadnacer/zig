...
pub const Base64Decoder = struct {
    ...
    const invalid_char_tst: u32 = 0x0f000000;
    const pad_char_tst: u32 = 0xff000000;
    ...
    pub fn init(alphabet_chars: [64]u8, pad_char: ?u8) Base64Decoder {
        ...
        if (pad_char) |pc| {
            for (0..4) |i| result.fast_char_to_index[i][pc] = pad_char_tst;
        }
    ...
    pub fn decode(decoder: *const Base64Decoder, dest: []u8, source: []const u8) Error!void {
        ...
        while (fast_src_idx + 15 < source.len and dest_idx + 15 < dest.len) : ({fast_src_idx += 16; dest_idx += 12;}) {
            var bits: u128 = 0;
            inline for (0..4) |i| {
                var new_bits: u128 = decoder.fast_char_to_index[0][source[fast_src_idx + i * 4]]
                                   | decoder.fast_char_to_index[1][source[fast_src_idx + 1 + i * 4]]
                                   | decoder.fast_char_to_index[2][source[fast_src_idx + 2 + i * 4]]
                                   | decoder.fast_char_to_index[3][source[fast_src_idx + 3 + i * 4]];
                if ((new_bits & invalid_char_tst) != 0) return error.InvalidCharacter;
                bits |= (new_bits << (24 * i));
            }
            std.mem.writeIntLittle(u128, dest[dest_idx..][0..16], bits);
        }
        while (fast_src_idx + 4 < source.len and dest_idx + 3 < dest.len) : ({fast_src_idx += 4; dest_idx += 3;}) { //TODO: CAN THIS BE +3? NOT HANDLING PADDING?
            const bits = decoder.fast_char_to_index[0][source[fast_src_idx]]
                       | decoder.fast_char_to_index[1][source[fast_src_idx + 1]]
                       | decoder.fast_char_to_index[2][source[fast_src_idx + 2]]
                       | decoder.fast_char_to_index[3][source[fast_src_idx + 3]];
...
fn testBase64() !void {
    ...
    try testNoSpaceLeftError(codecs, "foobarfoobarAA==");
}
...
fn testBase64UrlSafeNoPad() !void {
    ...
    try testNoSpaceLeftError(codecs, "AAAA");
    try testNoSpaceLeftError(codecs, "AAAAAA");
    try testNoSpaceLeftError(codecs, "foobarfoobarAAAA");
}
